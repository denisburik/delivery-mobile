...
export function CardHorizontalFood({ food }: { food: FoodProps }) {
  ...
  return (
    <Pressable className='flex flex-col rounded-xl relative'>
      ...
      <Text className='text-green-700 font-medium text-lg'>{formatPrice(food.price)}</Text>
      <Text className='text-black mt-1'>{food.name}</Text>
      <Text className='text-neutral-600 text-sm'>{food.time} - {formatPrice(food.delivery)}</Text>
   </Pressable>
  );
}